    ...
    return (
        <div className="w-full space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                {movers.map((item) => {
                    const isPositive = item.change30D == null ? true : item.change30D >= 0;
                    const hasChange30D = item.change30D != null;
                            ...
                            <Card className="bg-white/[0.03] border-white/5 hover:bg-white/[0.06] transition-all duration-300 overflow-hidden cursor-pointer h-full">
                                <CardContent className="!p-0">
                                    ...
                                    <div className={`absolute top-2 right-2 px-2 py-1 rounded-full text-xs font-bold flex items-center backdrop-blur-md shadow-lg ${
                                        ...
                                        {hasChange30D && (isPositive ? <ArrowUpRight className="w-3 h-3 mr-0.5" /> : <ArrowDownRight className="w-3 h-3 mr-0.5" />)}
                                ...
                                <div className="p-3 space-y-3">
                                    ...
                                    <div className="pt-3 flex justify-between items-center border-t border-white/10">
                                        <div className="flex flex-col">
                                            ...
                                            <span className={`text-xs font-bold mt-0.5 ${item.change12M == null ? 'text-gray-500' : item.change12M >= 0 ? 'text-success-text' : 'text-danger-text'}`}>
