...
import { TrendingUp, TrendingDown, Activity, Box, Zap } from "lucide-react";
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card className="bg-[#151A21] border border-[rgba(255,255,255,0.06)] shadow-none">
                <CardContent className="p-4 sm:p-6">
                    <div className="flex justify-between items-start gap-2">
                        <div className="min-w-0">
                            <p className="text-[10px] sm:text-xs font-semibold text-gray-500 uppercase tracking-wider">30D Momentum</p>
                            <h3 className="text-xl sm:text-2xl font-bold text-white mt-1.5 truncate">
                                {data.sealedIndex30D != null
                                    ? `${data.sealedIndex30D >= 0 ? '+' : ''}${data.sealedIndex30D.toFixed(1)}%`
                                    : '—'}
                            </h3>
                        </div>
                    ...
                    <div className={`mt-4 text-[11px] sm:text-sm font-medium leading-tight ${data.sealedIndex30D != null && data.sealedIndex30D >= 0 ? 'text-[#00E599]' : 'text-red-400'}`}>
                        {data.sealedIndex30D != null ? 'Avg sealed price change' : 'No data yet'}
                    </div>
                </CardContent>
            </Card>
